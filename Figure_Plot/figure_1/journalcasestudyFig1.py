...
Ideal-battery (M1-equivalent) formulation for Fig 1: continuous charging and
discharging powers Pc, Pd without binary mode variables.  With eta = 1 and
symmetric limits, the net power Pd - Pc spans [-Pbar, Pbar] and the model is
equivalent to the single-variable ideal-battery model M1 in the paper.
"""
#%% Global settings
...
if 'positive price case' not in info.sheetnames:
    raise KeyError(
        f"Worksheet 'positive price case' not found in {WORKBOOK_PATH}; "
        f"available sheets: {info.sheetnames}"
    )
setting_table_positive = info['positive price case'] # positive price data for Fig1
N_price = 6 # six stairs
pri_pos = np.zeros((N_price,N_t)) # initialize the price matrix
# read price data
for i in range(N_price):
    for j in range(N_t):
        pri_pos[i,j] = setting_table_positive.cell(row=i+2, column=j+2).value  # load the positive-case price of Fig 1
info.close()
